// FILE: netlify/functions/admin-review-upload.cjs
...
exports.handler = async (event) => {
  try {
    ...
    // Reviewed but not activated (kept out of Drip/Flow/Alignment)
    await batchRef.set(
      {
        adminStatus: "reviewed",
        adminReviewedAt: nowTS(),
        adminReviewedBy: adminUid,
        adminReviewNote: note || null,
        // Ensure activation is explicitly NOT active
        activation: {
          activated: false,
          decision: "reviewed",
          reviewedAt: nowTS(),
